...
def run_anchor_calibration_blocking(root: tk.Tk, *, modal: bool = True) -> bool:
    """
    Transparentes Vollbild-Overlay: Rahmen = Bildschirmkoordinaten.
    Speichern: Overlay ausblenden, ImageGrab des gewählten Rechtecks, PNG + JSON.
    """
    ...
    bf.place(relx=0.5, rely=1.0, y=-8, anchor="s")
    tk.Label(
        bf,
    ...
    rw_des = min(520, sw_i - 80)
    rh_des = min(380, sh_i - 160)
    ix0 = max(24, (sw_i - rw_des) // 2)
    iy0 = max(80, (sh_i - rh_des) // 3)
    ...
    def sync_origin_marker() -> None:
        bx1, by1, _, _ = canvas.coords(rect_id)
    ...
    def canvas_rect_to_xywh() -> tuple[int, int, int, int]:
        bx1, by1, bx2, by2 = canvas.coords(rect_id)
    ...
    wx0, wy0, ww0, hh0 = canvas_rect_to_xywh()
    ...
    def on_canvas_down(ev: tk.Event) -> None:
        ...
        if hx1 <= cx <= hx2 and hy1 <= cy <= hy2:
            ix_q, iy_q, w_q, h_q = canvas_rect_to_xywh()
            state["mode"] = "resize"
    ...
    def save_anchor() -> None:
        ix, iy, w_, h_ = canvas_rect_to_xywh()
        if w_ < 8 or h_ < 8:
